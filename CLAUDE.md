...
**Rate limiting keys on the account** for authenticated traffic and on the IP for everything else (`rateLimitKeyFor`), because carrier NAT puts thousands of subscribers behind one address. The bearer token is verified inside the key generator rather than read from `request.user`: `@fastify/rate-limit` installs a global `onRequest` hook that runs *before* a route's `onRequest: [authenticate]`, so `request.user` is still empty there. Login and the other unauthenticated endpoints stay on the IP — that is what their strict policies are for.
